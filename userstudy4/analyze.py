...
# 不再使用固定的 TARGET_IDS 列表；脚本会从数据中自动收集图片ID并按稳定顺序排序（若存在 'gt'，则放在首位）。
...
def get_all_images(data_list):
    """从 data_list 收集所有图片ID，返回稳定顺序列表。
    如果存在 'gt'，则将其放在第一位；其余按字母序排序。
    """
    imgs = set()
    for d in data_list:
        imgs.add(d['img_l'])
    ...
    if 'gt' in imgs:
        imgs.remove('gt')
        return ['gt'] + sorted(imgs)
    return sorted(imgs)

def _numeric_key_from_string(s):
    """尝试从字符串中提取数字作为排序键（如 'scene_001' -> 1），若没有数字则返回很大的数，使其排在后面。"""
...
def process_stats_and_matrices(data_list):
    ...
    for d in data_list:
        # 为避免 fov 或 mat 中包含下划线导致解析问题，full 维度使用 (fov, mat) 的 tuple 作为键
        keys = [
            ('full', (d['fov'], d['mat'])),
            ('mat',  d['mat']),
            ('fov',  d['fov'])
...
def export_all(data_list, output_dir, prefix):
    ...
    # 自动获取所有图片ID（'gt' 优先）
    all_imgs = get_all_images(data_list)
    ...
    with open(stats_path, 'w', newline='', encoding='utf-8-sig') as f:
        ...
        for fov_mat in sorted(processed['full']['stats'].keys(), key=lambda x: (_numeric_key_from_string(x[0]), x[1])):
            f_val, m_val = fov_mat
            for img in all_imgs:
        ...
        for fov in sorted(processed['fov']['stats'].keys(), key=_numeric_key_from_string):
            for img in all_imgs:
    ...
    with open(matrix_path, 'w', newline='', encoding='utf-8-sig') as f:
        ...
        for fov in sorted(processed['fov']['matrix'].keys(), key=_numeric_key_from_string):
            write_matrix_section(writer, "FOV", fov, processed['fov']['matrix'][fov], all_imgs)

        writer.writerow(["", "===== 交叉维度详细矩阵 ====="])
        for fov_mat in sorted(processed['full']['matrix'].keys(), key=lambda x: (_numeric_key_from_string(x[0]), x[1])):
            write_matrix_section(writer, "FOV_材质", f"{fov_mat[0]}_{fov_mat[1]}", processed['full']['matrix'][fov_mat], all_imgs)
