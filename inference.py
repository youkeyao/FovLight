...
def main(checkpoint_dir, model_param):
    ...
    # input_img = cv2.imread("/mnt/data/youkeyao/Datasets/LightEnv/scene_003/image.png", -1)[:, :, 0:3][:, :, ::-1].astype(np.float32) / 255
    ...
    # depth_img = torch.from_numpy(cv2.imread("/mnt/data/youkeyao/Datasets/LightEnv/scene_003/depth.exr", -1)[:, :, 0:1].astype(np.float32)).permute(2, 0, 1)
